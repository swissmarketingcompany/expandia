...
const SOLUTION_PAGE_BLUEPRINTS = {
    ...
    'ai-build-setup': {
        ...
        hero: {
            ...
            primaryCtaLink: 'contact.html',
            secondaryCtaText: 'Try the Microapps',
            secondaryCtaLink: '#microapps'
        },
        sections: [
            {
                type: 'cards',
            ...
            },
            {
                ...
                steps: [
                    ...
                    { title: 'We launch and improve it', description: 'Once people start using it, we refine the details and keep building from there.' }
                ]
            },
            {
                type: 'faq',
        ...
        cta: {
            ...
            description: 'We can start with one useful microapp, or we can build a larger workflow around it. The best first move depends on where the business value is.',
...
<section id="microapps" class="section-spacing bg-base-100">
    <div class="container mx-auto container-padding">
        <div class="text-center mb-12">
            ...
            <p class="text-base text-base-content/60 max-w-2xl mx-auto">Tap around. See what AI agents actually do.</p>
        </div>

        <div class="grid gap-6 lg:grid-cols-3">

            <!-- EXPENSE APPROVER -->
            <div class="buzz-card overflow-hidden bg-white shadow-xl" id="demo-expenses">
                <div class="px-5 pt-5 pb-4 border-b border-base-200">
                    <div class="flex items-center justify-between gap-3">
                        <h3 class="text-lg font-bold">Expense Approver</h3>
                        <span class="badge badge-primary badge-sm">Finance</span>
                    </div>
                </div>
                <div class="p-5">
                    <div class="flex items-center justify-between mb-4">
                        <div class="text-center">
                            <p class="text-3xl font-black text-primary" id="exp-approved">0</p>
                            <p class="text-[10px] uppercase tracking-widest text-base-content/40 mt-1">Approved</p>
                        </div>
                        <div class="text-center">
                            <p class="text-3xl font-black text-base-content/30" id="exp-pending">5</p>
                            <p class="text-[10px] uppercase tracking-widest text-base-content/40 mt-1">Pending</p>
                        </div>
                        <div class="text-center">
                            <p class="text-3xl font-black text-red-400" id="exp-flagged">0</p>
                            <p class="text-[10px] uppercase tracking-widest text-base-content/40 mt-1">Flagged</p>
                        </div>
                    </div>

                    <div id="exp-queue" class="space-y-2">
                    </div>

                    <div id="exp-done-msg" class="hidden text-center py-6">
                        ...
                        <p class="font-bold text-base-content/80">Queue cleared</p>
                        <button type="button" id="exp-reset" class="btn btn-xs btn-ghost text-primary mt-2">Reset demo</button>
                    </div>

                    <div class="mt-4 rounded-xl bg-base-200/60 px-3 py-2 flex items-center justify-between">
                        <p class="text-xs text-base-content/50">Total approved</p>
                        <p class="text-sm font-bold text-primary" id="exp-total">EUR 0</p>
                    </div>
                </div>
            </div>

            <!-- SUPPORT TRIAGE -->
            <div class="buzz-card overflow-hidden bg-white shadow-xl" id="demo-triage">
                <div class="px-5 pt-5 pb-4 border-b border-base-200">
                    <div class="flex items-center justify-between gap-3">
                        <h3 class="text-lg font-bold">Support Triage</h3>
                        <span class="badge badge-secondary badge-sm">Operations</span>
                    </div>
                </div>
                <div class="p-5">
                    <div id="triage-inbox" class="space-y-2 mb-4">
                    </div>

                    <div id="triage-running" class="hidden text-center py-4">
                        <div class="w-10 h-10 border-4 border-primary/30 border-t-primary rounded-full animate-spin mx-auto mb-3"></div>
                        <p class="text-sm font-medium text-base-content/60">Sorting tickets…</p>
                    </div>

                    <div id="triage-result" class="hidden space-y-3">
                        <div class="flex items-center gap-3">
                            <span class="w-2.5 h-2.5 rounded-full bg-red-400 flex-shrink-0"></span>
                            <div class="flex-1 h-2 bg-base-200 rounded-full overflow-hidden"><div class="h-full bg-red-400 rounded-full transition-all duration-700" id="bar-urgent" style="width:0%"></div></div>
                            <span class="text-xs font-bold text-red-400 w-8 text-right" id="count-urgent">0</span>
                        </div>
                        <div class="flex items-center gap-3">
                            <span class="w-2.5 h-2.5 rounded-full bg-amber-400 flex-shrink-0"></span>
                            <div class="flex-1 h-2 bg-base-200 rounded-full overflow-hidden"><div class="h-full bg-amber-400 rounded-full transition-all duration-700" id="bar-medium" style="width:0%"></div></div>
                            <span class="text-xs font-bold text-amber-400 w-8 text-right" id="count-medium">0</span>
                        </div>
                        <div class="flex items-center gap-3">
                            <span class="w-2.5 h-2.5 rounded-full bg-green-400 flex-shrink-0"></span>
                            <div class="flex-1 h-2 bg-base-200 rounded-full overflow-hidden"><div class="h-full bg-green-400 rounded-full transition-all duration-700" id="bar-low" style="width:0%"></div></div>
                            <span class="text-xs font-bold text-green-400 w-8 text-right" id="count-low">0</span>
                        </div>
                        <div class="mt-2 rounded-xl bg-base-200/60 px-3 py-2 flex items-center justify-between">
                            <p class="text-xs text-base-content/50">Time saved</p>
                            <p class="text-sm font-bold text-secondary" id="triage-saved">~45 min</p>
                        </div>
                        <button type="button" id="triage-reset" class="btn btn-xs btn-ghost text-secondary w-full mt-1">Reset demo</button>
                    </div>

                    <button type="button" id="triage-run" class="btn btn-secondary btn-sm w-full">Sort 8 tickets</button>
                </div>
            </div>

            <!-- INVOICE SCANNER -->
            <div class="buzz-card overflow-hidden bg-white shadow-xl" id="demo-invoices">
                <div class="px-5 pt-5 pb-4 border-b border-base-200">
                    <div class="flex items-center justify-between gap-3">
                        <h3 class="text-lg font-bold">Invoice Scanner</h3>
                        <span class="badge badge-accent badge-sm">Accounting</span>
                    </div>
                </div>
                <div class="p-5">
                    <div id="inv-stack" class="grid grid-cols-3 gap-2 mb-4">
                    </div>

                    <div id="inv-scanning" class="hidden text-center py-4">
                        <div class="w-10 h-10 border-4 border-accent/30 border-t-accent rounded-full animate-spin mx-auto mb-3"></div>
                        <p class="text-sm font-medium text-base-content/60">Reading invoice…</p>
                    </div>

                    <div id="inv-extracted" class="hidden">
                        <div class="rounded-xl bg-base-200/60 p-3 space-y-2">
                            <div class="flex justify-between"><span class="text-xs text-base-content/50">Vendor</span><span class="text-xs font-bold" id="inv-vendor">—</span></div>
                            <div class="flex justify-between"><span class="text-xs text-base-content/50">Amount</span><span class="text-xs font-bold text-accent" id="inv-amount">—</span></div>
                            <div class="flex justify-between"><span class="text-xs text-base-content/50">Due</span><span class="text-xs font-bold" id="inv-due">—</span></div>
                            <div class="flex justify-between"><span class="text-xs text-base-content/50">Category</span><span class="text-xs font-bold" id="inv-cat">—</span></div>
                        </div>
                        <div class="flex items-center gap-2 mt-3 justify-center">
                            <span class="w-2 h-2 rounded-full bg-green-400"></span>
                            <span class="text-xs text-green-600 font-medium">Auto-matched to ERP</span>
                        </div>
                    </div>

                    <div class="mt-4 rounded-xl bg-base-200/60 px-3 py-2 flex items-center justify-between">
                        <p class="text-xs text-base-content/50">Scanned today</p>
                        <p class="text-sm font-bold text-accent" id="inv-count">0 / 6</p>
                    </div>
                </div>
            </div>
    ...
    <style>
        #demo-expenses .exp-row { display:flex; align-items:center; gap:0.5rem; padding:0.5rem 0.75rem; border-radius:0.75rem; background:oklch(0.97 0 0); transition:all 0.3s ease; }
        #demo-expenses .exp-row.done { opacity:0; max-height:0; padding:0; margin:0; overflow:hidden; }
        #demo-expenses .exp-icon { width:2rem; height:2rem; border-radius:0.5rem; display:flex; align-items:center; justify-content:center; font-size:0.8rem; flex-shrink:0; }
        #demo-expenses .exp-label { flex:1; font-size:0.8rem; font-weight:600; color:oklch(0.35 0 0); }
        #demo-expenses .exp-amt { font-size:0.8rem; font-weight:700; color:oklch(0.35 0 0); margin-right:0.25rem; }
        #demo-expenses .exp-btn { width:1.75rem; height:1.75rem; border-radius:50%; border:none; cursor:pointer; display:flex; align-items:center; justify-content:center; font-size:0.7rem; transition:transform 0.15s; }
        #demo-expenses .exp-btn:hover { transform:scale(1.15); }
        #demo-expenses .exp-yes { background:oklch(0.85 0.15 145); color:oklch(0.35 0.15 145); }
        #demo-expenses .exp-no { background:oklch(0.90 0.10 25); color:oklch(0.45 0.15 25); }
        @keyframes pop-in { from { transform:scale(0.8); opacity:0; } to { transform:scale(1); opacity:1; } }
        #demo-triage .triage-ticket { display:flex; align-items:center; gap:0.5rem; padding:0.5rem 0.75rem; border-radius:0.75rem; background:oklch(0.97 0 0); font-size:0.75rem; font-weight:500; color:oklch(0.45 0 0); }
        #demo-invoices .inv-doc { aspect-ratio:3/4; border-radius:0.75rem; display:flex; align-items:center; justify-content:center; font-size:0.65rem; font-weight:600; cursor:pointer; transition:all 0.2s; border:2px solid transparent; }
        #demo-invoices .inv-doc:hover { transform:translateY(-2px); box-shadow:0 4px 12px oklch(0 0 0/0.1); }
        #demo-invoices .inv-doc.scanned { border-color:oklch(0.7 0.15 160); background:oklch(0.95 0.05 160)!important; color:oklch(0.4 0.1 160)!important; }
    </style>
    ...
    <script>
        (() => {
            /* ── EXPENSE APPROVER ── */
            const expenses = [
                { label: 'Team lunch', amount: 340, icon: '🍽️', bg: 'oklch(0.92 0.08 80)' },
                { label: 'SaaS license', amount: 1200, icon: '💻', bg: 'oklch(0.90 0.08 260)' },
                { label: 'Flight BER→ZRH', amount: 489, icon: '✈️', bg: 'oklch(0.92 0.06 200)' },
                { label: 'Office supplies', amount: 95, icon: '📎', bg: 'oklch(0.93 0.06 50)' },
                { label: 'Client dinner', amount: 780, icon: '🥂', bg: 'oklch(0.91 0.08 350)' }
            ];
            let expApproved = 0, expFlagged = 0, expTotal = 0, expPending = expenses.length;
            const expQueue = document.getElementById('exp-queue');
            const expDoneMsg = document.getElementById('exp-done-msg');

            function renderExpenses() {
                expQueue.innerHTML = expenses.map((e, i) =>
                    '<div class="exp-row" id="exp-r-' + i + '">' +
                    '<div class="exp-icon" style="background:' + e.bg + '">' + e.icon + '</div>' +
                    '<span class="exp-label">' + e.label + '</span>' +
                    '<span class="exp-amt">€' + e.amount + '</span>' +
                    '<button class="exp-btn exp-yes" data-idx="' + i + '" data-act="yes">✓</button>' +
                    '<button class="exp-btn exp-no" data-idx="' + i + '" data-act="no">✕</button>' +
                    '</div>'
                ).join('');
            }
            renderExpenses();
            ...
            function updateExpCounters() {
                document.getElementById('exp-approved').textContent = expApproved;
                document.getElementById('exp-pending').textContent = expPending;
                document.getElementById('exp-flagged').textContent = expFlagged;
                document.getElementById('exp-total').textContent = 'EUR ' + expTotal.toLocaleString();
            }

            expQueue.addEventListener('click', function(e) {
                const btn = e.target.closest('.exp-btn');
                if (!btn) return;
                const idx = parseInt(btn.dataset.idx);
                const act = btn.dataset.act;
                const row = document.getElementById('exp-r-' + idx);
                if (!row || row.classList.contains('done')) return;

                if (act === 'yes') { expApproved++; expTotal += expenses[idx].amount; }
                else { expFlagged++; }
                expPending--;
                ...
                if (expPending <= 0) {
                    setTimeout(function() { expQueue.classList.add('hidden'); expDoneMsg.classList.remove('hidden'); }, 350);
                }
            });

            document.getElementById('exp-reset').addEventListener('click', function() {
                expApproved = 0; expFlagged = 0; expTotal = 0; expPending = expenses.length;
                updateExpCounters();
                expDoneMsg.classList.add('hidden');
                expQueue.classList.remove('hidden');
                renderExpenses();
            });
            ...
            /* ── SUPPORT TRIAGE ── */
            const tickets = [
                { label: 'Server down – EU', pri: 'urgent' },
                { label: 'Login issue', pri: 'medium' },
                { label: 'Password reset', pri: 'low' },
                { label: 'API timeout', pri: 'urgent' },
                { label: 'Billing question', pri: 'low' },
                { label: 'Data export fail', pri: 'medium' },
                { label: 'SSL cert expiry', pri: 'urgent' },
                { label: 'UI bug report', pri: 'low' }
            ];
            const priColors = { urgent: 'oklch(0.75 0.15 25)', medium: 'oklch(0.80 0.12 80)', low: 'oklch(0.82 0.12 145)' };
            const triageInbox = document.getElementById('triage-inbox');
            const triageRun = document.getElementById('triage-run');
            const triageRunning = document.getElementById('triage-running');
            const triageResult = document.getElementById('triage-result');

            function renderInbox() {
                triageInbox.innerHTML = tickets.map(function(t) {
                    return '<div class="triage-ticket"><span style="width:6px;height:6px;border-radius:50%;background:oklch(0.7 0 0);flex-shrink:0"></span>' + t.label + '</div>';
                }).join('');
            }
            renderInbox();

            triageRun.addEventListener('click', function() {
                triageRun.classList.add('hidden');
                triageInbox.classList.add('hidden');
                triageRunning.classList.remove('hidden');

                setTimeout(function() {
                    triageRunning.classList.add('hidden');
                    triageResult.classList.remove('hidden');

                    var counts = { urgent: 0, medium: 0, low: 0 };
                    tickets.forEach(function(t) { counts[t.pri]++; });
                    var total = tickets.length;

                    setTimeout(function() {
                        document.getElementById('bar-urgent').style.width = (counts.urgent / total * 100) + '%';
            ...
            document.getElementById('triage-reset').addEventListener('click', function() {
                ...
                triageInbox.classList.remove('hidden');
                triageRun.classList.remove('hidden');
                document.getElementById('bar-urgent').style.width = '0%';
                document.getElementById('bar-medium').style.width = '0%';
                document.getElementById('bar-low').style.width = '0%';
                renderInbox();
            });
            ...
            /* ── INVOICE SCANNER ── */
            const invoices = [
                { vendor: 'TechParts GmbH', amount: 'EUR 4,320', due: 'Apr 12', cat: 'Hardware', color: 'oklch(0.90 0.08 260)' },
                { vendor: 'CloudServ AG', amount: 'EUR 1,850', due: 'Apr 15', cat: 'SaaS', color: 'oklch(0.90 0.08 200)' },
                { vendor: 'OfficeMax Ltd', amount: 'EUR 620', due: 'Apr 18', cat: 'Supplies', color: 'oklch(0.92 0.06 80)' },
                { vendor: 'LegalPro SA', amount: 'EUR 3,100', due: 'Apr 20', cat: 'Services', color: 'oklch(0.90 0.08 350)' },
                { vendor: 'DataLink Corp', amount: 'EUR 7,500', due: 'Apr 22', cat: 'Infrastructure', color: 'oklch(0.88 0.08 145)' },
                { vendor: 'PrintWorks KG', amount: 'EUR 290', due: 'Apr 25', cat: 'Marketing', color: 'oklch(0.92 0.06 50)' }
            ];
            let invScanned = 0;
            const invStack = document.getElementById('inv-stack');
            const invScanning = document.getElementById('inv-scanning');
            const invExtracted = document.getElementById('inv-extracted');

            function renderInvStack() {
                invStack.innerHTML = invoices.map(function(inv, i) {
                    var cls = i < invScanned ? ' scanned' : '';
                    return '<div class="inv-doc' + cls + '" data-inv="' + i + '" style="background:' + (i < invScanned ? '' : inv.color) + ';color:oklch(0.45 0 0)">' +
                        '<span>' + (i < invScanned ? '✓' : 'PDF') + '</span></div>';
                }).join('');
            }
            renderInvStack();

            invStack.addEventListener('click', function(e) {
                var doc = e.target.closest('.inv-doc');
                if (!doc) return;
                var idx = parseInt(doc.dataset.inv);
                if (idx !== invScanned) return;

                invExtracted.classList.add('hidden');
                invScanning.classList.remove('hidden');

                setTimeout(function() {
                    invScanning.classList.add('hidden');
                    var inv = invoices[idx];
                    document.getElementById('inv-vendor').textContent = inv.vendor;
                    document.getElementById('inv-amount').textContent = inv.amount;
                    document.getElementById('inv-due').textContent = inv.due;
                    document.getElementById('inv-cat').textContent = inv.cat;
                    invExtracted.classList.remove('hidden');
                    invExtracted.style.animation = 'pop-in 0.3s ease';

                    invScanned++;
                    document.getElementById('inv-count').textContent = invScanned + ' / ' + invoices.length;
                    renderInvStack();
                }, 900);
            });
        })();
